...
void Collection::add(TElem elem)
{
	if (search(elem))
	{
		...
		for (int i = 0; i < this->distinctelements && ok; i++)
			if (this->elements[i].valoare == elem) this->elements[i].nr_aparitii++, ok = 0;
	}
	else
	{
...
bool Collection::remove(TElem elem)
{
	...
	{
		int i = 0;
		while (this->elements[i].valoare != elem) i++;
...
bool Collection::search(TElem elem)
{
	for (int i = 0; i < this->distinctelements; i++)
		if (this->elements[i].valoare == elem) return true;
	return false;
}
...
int Collection::getLungime()
{
	return this->distinctelements;
}

void Collection::setNumarBancnote(Element elem)
{
	if (search(elem.valoare))
	{
		int i = 0;
		while (this->elements[i].valoare != elem.valoare) i++;
		this->elements[i].nr_aparitii = elem.nr_aparitii;
	}
}

TElem Collection::getVal(int position)
{
